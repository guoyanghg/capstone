...
extension UIImage {
  ...
  func scaledData(with size: CGSize, byteCount: Int, isQuantized: Bool) -> Data? {
    ...
    //normalize image by 0-255 cv.NORM_MIN_MAX
    let min = scaledBytes.min()
    let max = scaledBytes.max()
    
    guard let unwrapedmin = min else {
        return nil
    }
    
    guard let unwrapedmax = max else {
        return nil
    }
    let fmax = Float(unwrapedmax)
    let fmin = Float(unwrapedmin)
    let range = fmax - fmin
    //print(scaledBytes[0])
    let scaledFloats = scaledBytes.map { (Float32($0) - fmin)*255 / range}
    //print(scaledFloats[0])
