...
class ImageSegmentator {
  ...
  func runSegmentation(
    ...
    tfLiteQueue.async {
      ...
      // Processing Unet tensor
      let unetOutput = self.parseOutputTensorUnet(outputTensor: outputTensor)
      ...
      // visualization.
      let colorLegend = self.classListToColorLegend(classList: [0,1])
  ...
  private func parseOutputTensorUnet(outputTensor: Tensor)
    -> [UInt32]
  {
    ...
    for x in 0..<self.outputImageWidth {
      for y in 0..<self.outputImageHeight {
        ...
        var legendColor = Constants.legendColorList[0]
        if valMax < 0.1 {
            legendColor = Constants.legendColorList[1]
        }
        segmentationImagePixels[x * self.outputImageHeight + y] = legendColor
      }
    }

    return segmentationImagePixels
  }
...
private enum Constants {
  ...
  static let modelFileName = "unetmodel"
  //static let modelFileName = "deeplabv3_257_mv_gpu"
  //static let unetModelFileName = 'unetmodel'
