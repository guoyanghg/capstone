...
class ViewController: UIViewController {
  ...
  @IBAction func onSave(_ sender: UIButton) {
    ...
    guard self.segmentationResult != nil else {
      self.inferenceStatusLabel.text = "Error: Result is nil."
      return
    }
    
    UIImageWriteToSavedPhotosAlbum(self.segmentationResult!.overlayImage, nil, nil, nil)
    UIImageWriteToSavedPhotosAlbum(self.segmentationResult!.resultImage, nil, nil, nil)
...
extension ViewController {
  ...
  private func showDemoSegmentation() {
    if let filePath = Bundle.main.path(forResource: "nuc2", ofType: "png"),
